"""Custom Layers for a Conditional Generative Adversarial Network

This module contains custom layers used in the Conditional Generative Adversarial Network Architecture.
...
class Generator(nn.Module):
    ...
    This class implements the Conditional GAN's generator module. It takes an input tensor of shape
    ...
    Parameters:
        ...
        img_size: (int): The image dimension
    ...
    def __init__(self, latent_dim: int, mid_channels: int, out_channels: int, num_labels: int, img_size: int, embed_size: int) -> None:
        super().__init__()
        self.img_size = img_size
        self.model = nn.Sequential(
            # Output Shape: [1, 1024, 2, 2]
            *self.generator_block(latent_dim + embed_size, mid_channels * 16),
    ...
    def forward(self, x, labels):
        ...
        Parameters:
            labels (torch.Tensor): Input tensor
            x (torch.Tensor): Input tensor of shape (batch_size, latent_dim, 1, 1).
        ...
        return self.model(torch.cat([x, embeddings], dim=1))


class Critic(nn.Module):
    """
    Critic Module for the Conditional GAN.

    This class implements the Conditional GAN's Critic module. It takes an input tensor of shape
    ...
    def __init__(self, in_channels: int, mid_channels: int, out_channels: int, num_classes: int, img_size: int):
        super().__init__()
        self.img_size = img_size
        self.model = nn.Sequential(
            # Output Shape: [1, 64, 32, 32]
            *self.critic_block(in_channels + 1, mid_channels, normalize=False),
            # Output Shape: [1, 128, 16, 16]
    ...
    def forward(self, x, labels):
        ...
        return self.model(torch.cat([x, embedding], dim=1))
