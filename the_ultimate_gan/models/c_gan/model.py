"""
This module defines the CGAN model architecture and its different functions.

Class:
    CGAN: A class that implements the CGAN Model
"""
...
from the_ultimate_gan.models.c_gan.layers import Critic, Generator
...
class CGAN:
    ...
    This class implements the C GAN model which is used to generate images from random noise.
    ...
    Attributes:
        ...
        - num_labels (int): The total number of labels
        - embed_size (int): The size of label embeddings
        - lambda_gradient_penalty (int): The gradient penalty for the critic
    ...
    def __init__(
        ...
        print(
            f"Model C GAN Loaded with dataset: {dataset}. The following configurations were used:\nLearning Rate: {learning_rate}, Epochs: {num_epochs}, Batch Size: {batch_size}, Transforms with Mean: 0.5 and Std: 0.5 for each Channel.\n Starting the Model Training now."
        )

    def init_generator(self, latent_dim, out_channels):
        # Initialize the generator
        self.generator = Generator(latent_dim, 64, out_channels, self.num_labels, self.img_size, self.embed_size).to(self.device)
        self.generator.apply(weights_init)

    def init_critic(self, in_channels):
        # Initialize the critic
        self.critic = Critic(in_channels, 64, 1, self.num_labels, self.img_size).to(self.device)
        self.critic.apply(weights_init)

    def init_optimizers(self, learning_rate):
        # Initialize the critic and generator optimizer
    ...
    def train(self):
        """
        Train the C GAN model using the dataset.

        This method trains the C GAN model using the dataset provided in the constructor.
        ...
        try:
            ...
            while self.current_epoch <= self.num_epochs:
                for batch_idx, (real, labels) in enumerate(tqdm(self.loader)):  # Get the inputs; data is a list of [inputs, labels]
                    ...
                    for i in range(self.critic_steps):
                        ...
                        fake = self.generator(noise, labels)  # Generate fake images

                        critic_real = self.critic(real, labels)
                        critic_fake = self.critic(fake, labels)
                        gp = gradient_penalty(self.critic, real, fake, device=self.device, labels=labels)
                    ...
                    output = self.critic(fake, labels)  # Get the critic output for fake images
